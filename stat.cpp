...
char buf[1000]; //input buffer
  ...
  for(i=0;i<N;i++){//loop over the rows of the file
    ...
    for(j=1;j<M;j++){//loop over the rest of the row
      while(buf[k]!=' ')k++;while(buf[k]==' ')k++; //find next entry
...
int i,rp,sw;
  ...
  for(i=0;i<tz;i++){//select tournament members
    ...
    sw=dx[i];dx[i]=dx[rp];dx[rp]=sw; //swap it in
  }

  do {//sort the tournament into lowest fitness first order
    rp=0;  //reset flag
    for(i=0;i<tz-1;i++)if(fit[dx[i]]>fit[dx[i+1]]){//if out of order
      sw=dx[i];dx[i]=dx[i+1];dx[i+1]=sw; //swap
      rp=1;  //set flag
    }
...
int i,sw;
  ...
  for(i=0;i<N;i+=2){//loop over pairs
    if(fit[dx[i]]>fit[dx[i+1]]){//if the pair is out of order
      sw=dx[i];dx[i]=dx[i+1];dx[i+1]=sw;  //put them in order
    }
  }
}
...
int FPS(double *fit,double ttl,int n){//perform fitness proportional selection

int i,p;
double dart;

  if(ttl==0)for(i=0;i<n;i++)ttl+=fit[i];
  p=0;dart=drand48()*ttl-fit[0];
  while(p<n){
    if(dart<0)return(p);
    dart-=fit[++p];
  }
